...
> ⚠️ **위 정확도·지연은 안전 파이프라인 보강(게이트 3단계·확인 만료·상태 재검사·메시지 본문 필수)
> 이전 코드에서 잰 값이다.** 기대값이 바뀐 케이스는 없지만, 메시지 본문이 없으면 되묻도록 바꿨으므로
> 모델이 본문을 안 채우는 경우 결과가 달라질 수 있다. `--eval`을 다시 돌리기 전까지 이 수치를
> "현재 코드의 실측"으로 인용하지 않는다.

## 최종 결과

> 아래는 **안전 파이프라인 보강 이전** 코드 기준이다. 재실행 전까지 현재 코드의 실측이 아니다.
