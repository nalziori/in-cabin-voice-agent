...
이 에이전트의 워크로드는 특이하다. 출력은 `Intent` JSON 하나라 **디코드가 60토큰 남짓**인데,
입력은 SYSTEM이 **2,276토큰**이다(실측 — 프롬프트 캐시 절 참고).

프리필을 매번 하면 1B라도 2,400토큰에 **2.7~4.8초**가 걸린다. 모델을 아무리 줄여도 안 된다.

해결책은 이미 코드에 있다. **SYSTEM이 고정이고 매 요청 바뀌는 값은 user 쪽에 붙인다**는 규칙
(`CLAUDE.md` 불변 조건) 덕분에, 시동 시 SYSTEM의 KV 캐시를 한 번 만들어 두고 발화마다 새로 들어오는
~100토큰만 프리필하면 된다. 클라우드에서 `cache_read=2276`으로 나타난 이득이, 온디바이스에서는
KV 재사용으로 나타난다 — **같은 설계 결정이 두 환경에서 각각 효과를 낸다.**

디코드도 줄일 수 있다: null 필드를 빼고, 문법 제약 디코딩으로 JSON 구조 토큰을 강제하면 60 → 30토큰대.

| 구성 | 프리필 | 디코드 | 합계(추정) |
|---|---|---|---|
| 1B INT4, KV 재사용 없음 | 2.7~4.8s | 0.5~0.8s | **3~6s — 불가** |
| 1B INT4, KV 재사용 | 0.1~0.2s | 0.4~1.0s | **0.5~1.2s — 가능** |
| 3B INT4, KV 재사용 | 0.3~0.5s | 1.5~2.4s | 2~3s — 느리다 |

로컬 전사 0.67초(실측)가 앞에 붙으므로 종단 1.2~1.9초. 차 안 음성 응답으로 쓸 만한 경계선이다.
...
측정된 것은 **SYSTEM 2,276토큰**과 **로컬 전사 0.67초**뿐이다. 나머지 tok/s·초·와트는 전부 공개
스펙에서 계산한 추정이고, **이 프로젝트는 Jetson에서 아무것도 돌려보지 않았다.** 실기를 구하면
1B INT4를 올려 프리필·디코드·전력을 실측하는 것이 다음 단계다.
...
- **온디바이스는 가정이고, 그 가정의 하드웨어 비용을 실측한 적이 없다.** 의도 추론은 현재 클라우드
  ...
  SYSTEM 2,276토큰의 프리필은 KV 캐시를 시동 시 미리 만들어 둔다는 전제에서만 성립하는데,
  그 재사용을 구현하지도 측정하지도 않았다 — 지금 구조에서 온디바이스 실현성의 최대 미지수다.
