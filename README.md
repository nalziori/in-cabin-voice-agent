...
| **실행 간 변이** | **관측됨** — tune 3회 중 1회에서 `'등받이를 뒤로 20도까지 눕혀줘'` 1건 오분류(tool 92.9%) |

측정하지 않은 것은 측정하지 않았다고 적는다. 수치는 실제로 돌린 뒤에 채운다.

위 수치는 **메시지 본문 채점과 값 방향 규약을 넣은 뒤 다시 잰 값이다**(2026-09-07 3차).

> ⚠️ **100%는 한 번 돌린 결과다.** 같은 조건으로 tune을 3회 돌렸을 때 1회에서
> `'등받이를 뒤로 20도까지 눕혀줘'`가 오분류됐다(tool 92.9%). n=14에서 1건은 7%p라
> 단일 실행 100%로는 안정성을 주장할 수 없다. **반복 측정이 남은 일이다.**
...
- **정확도를 한 번만 재고 있다.** 3차 측정에서 같은 케이스가 실행에 따라 맞기도 틀리기도 했다
  (tune 3회 중 1회 오분류). 단일 실행 100%는 안정성의 근거가 못 된다 — 반복 측정과 분산 보고가
  필요하다. 지금 숫자는 "이 실행에서 틀리지 않았다"까지만 뜻한다.
