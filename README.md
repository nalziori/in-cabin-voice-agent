...
- **화자 구분이 없다.** 누가 말했는지(운전석·조수석·뒷좌석) 구분하지 않는다. `_resolve()`의
  시트 조작은 `zone`이 실제로는 항상 `"driver"`로 고정돼 있어서, 뒷좌석 승객이 "시트 좀
  세워줘"라고 말해도 운전석이 조작된다 — 코드 리뷰 중 발견한, README 기능 표에는 드러나지
  않던 실제 동작 제약이다.
- **ASR 출력에 신뢰도 검증이 없다.** `transcribe()`는 faster-whisper가 뱉은 텍스트를 그대로
  다음 단계로 넘긴다. 모델 출력은 신뢰 경계로 취급해 값 범위를 자르지만(`LIMITS`), 그 앞단인
  ASR 오독(예: "3도"를 "8도"로 잘못 들음)은 범위 안에 있으면 걸러낼 방법이 없다 — 두 번째
  신뢰 경계가 비어 있다.
